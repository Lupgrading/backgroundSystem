    ...
    <el-table style="width: 100%" border :data="records">
      ...
      <el-table-column prop="prop" label="操作" width="width">
        <template slot-scope="{ row, $index }">
          ...
          ></el-button>
          <el-popconfirm title="这是一段内容确定删除吗？" @onConfirm="">
            <hint-button
              slot="reference"
              type="danger"
              icon="el-icon-delete"
        ...
        <el-col :span="16"
          ><el-carousel height="500px">
            <el-carousel-item
              ...
              :key="item.id"
            >
              <img :src="item.imgUrl" style="width: 100%; height: 500px" />
...
export default {
  ...
  methods: {
    ...
    async getSkuList(pages = 1) {
      ...
      let reslut = await this.$API.sku.reqSkuList(page, limit);
      if (reslut.code == 200) {
    ...
    //上架
    async sale(row) {
      let reslut = await this.$API.sku.reqSale(row.id);
      if ((reslut.code = 200)) {
        row.isSale = 1;
